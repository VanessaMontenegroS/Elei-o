...
# corresponda a nenhum candidato ou seja branco, ele deve ser tratado como nulo. Se for inserido um texto ao
# invés de número, o código deve retornar uma mensagem para votar novamente.
...
while resp == 'N':
    ...
    try:
        ...
        print(' ')
        print(f'O valor digitado não é válido, tente novamente.')
        print(' ')
        painel()
        ...
        painel()
        print(' ')
        resp = str(input('Deseja encerrar a votação? [S/N]')).upper().strip()[0]
        if resp not in 'SN':
...
if cont_Y > cont_X and cont_Y > cont_Z and cont_Y > cont_B and cont_Y > cont_N:
    maior = cont_Y
    print(f'Vencedor foi o CANDIDATO Y com {cont_Y} votos.')

else:

    if cont_Z > cont_X and cont_Z > cont_Y and cont_Z > cont_B and cont_Z > cont_N:
        maior = cont_Z
        print(f'Vencedor foi o CANDIDATO Z com {cont_Z} votos.')

    if cont_X > cont_Y and cont_X > cont_Z and cont_X > cont_B and cont_X > cont_N:
        maior = cont_X
        print(f'Vencedor foi o CANDIDATO X com {cont_X} votos.')